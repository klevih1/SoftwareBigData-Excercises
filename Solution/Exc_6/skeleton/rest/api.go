...
func GetReceiptFile(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// get order from db
		dbOrder, err := db.GetOrder(uintId)
		if err != nil {
		...
		if err != nil {
			slog.Error("S3 GetObject failed", slog.String("error", err.Error()))
			render.Status(r, http.StatusInternalServerError)
		...
		w.Header().Set("Content-Disposition", "attachment; filename=\""+dbOrder.GetFilename()+"\"")
